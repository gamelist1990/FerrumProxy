...
**ローカルループバック echo で 1000 メッセージ × 512 バイト payload、3 回連続計測の中央値** (自前計測ツール `benches_udp/` で誰でも再現可、Python 3 標準ライブラリのみ / Windows・Ubuntu 共通):

#### UDP (Bedrock の基盤プロトコル)

| 指標 | Baseline (直接) | FerrumProxy 経由 | オーバーヘッド |
|---|---:|---:|---:|
| **Mean 遅延** | 0.027 ms | **0.049 ms** | **+0.022 ms** |
| **P50** | 0.023 ms | 0.044 ms | +0.021 ms |
| **P90** | 0.029 ms | 0.057 ms | +0.028 ms |
| **P99** | 0.095 ms | 0.126 ms | +0.031 ms |
| **Total (1000 msg)** | 26.8 ms | 49.1 ms | **+22.3 ms** |
...
| 指標 | Baseline (直接) | FerrumProxy 経由 | オーバーヘッド |
|---|---:|---:|---:|
| **Mean 遅延** | 0.024 ms | **0.047 ms** | +0.023 ms |
| **P50** | 0.022 ms | 0.042 ms | +0.020 ms |
| **P90** | 0.025 ms | 0.052 ms | +0.027 ms |
| **P99** | 0.062 ms | 0.117 ms | +0.055 ms |
| **Total (1000 msg)** | 24.0 ms | 47.0 ms | **+23.0 ms** |
| **ロス** | 0/1000 | **0/1000** | 0% |

> **1 メッセージあたり proxy 由来のオーバーヘッドは わずか 22〜23 マイクロ秒** (=0.000022 秒)。
